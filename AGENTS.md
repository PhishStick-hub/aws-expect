...
- **`aws_expect/lambda_function.py`** — `LambdaFunctionExpectation`; uses boto3 **client** (no resource API). `to_respond_with` is not a waiter — asserts immediately; raises `LambdaResponseMismatchError` with a `reason` kwarg (`invalid_status_code`, `function_error`, `empty_payload`, `invalid_json`, `invalid_payload_type`, `payload_mismatch`)
